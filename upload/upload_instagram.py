...
def upload_to_instagram(video_path, caption, is_story=False, thumb_path=None):
    ...
    print("\n" + "=" * 60)
    print("ðŸ“¸ INSTAGRAM UPLOAD STARTING")
    print("=" * 60)
    ...
    if not access_token:
        error_msg = "âŒ INSTAGRAM_ACCESS_TOKEN not set"
        print(f"[instagram] {error_msg}")
        raise ValueError(error_msg)

    if not user_id:
        error_msg = "âŒ INSTAGRAM_ACCOUNT_ID not set"
        print(f"[instagram] {error_msg}")
        raise ValueError(error_msg)

    print(f"[instagram] âœ… Credentials loaded")
    ...
    try:
        ...
        if temp_response.status_code != 200:
            ...
            print(f"[instagram] âŒ {error_msg}")
        ...
        print(f"[instagram] ðŸ“¦ Step 2: Creating Instagram {media_type} container...")
        ...
        if container_response.status_code != 200:
            ...
            print(f"[instagram] âŒ Container creation failed: {error_msg}")
            print(f"[instagram] Full response: {container_response.text[:500]}")
            raise Exception(f"Instagram Container Error: {error_msg}")

        container_id = container_response.json().get('id')
        print(f"[instagram] âœ… Container created: {container_id}")

        # Step 3: Wait for processing
        print(f"[instagram] â³ Step 3: Waiting for video processing...")
        max_wait = 120
        waited = 0
        ...
        while waited < max_wait:
            ...
            if status_code == 'FINISHED':
                print(f"[instagram] âœ… Video processing complete!")
                break
        ...
        # Step 4: Publish
        print(f"[instagram] ðŸ“¤ Step 4: Publishing to Instagram...")
        ...
        if publish_response.status_code != 200:
            ...
            error_msg = error_data.get('error', {}).get('message', 'Unknown error')
            print(f"[instagram] âŒ Publish failed: {error_msg}")
            raise Exception(f"Instagram Publish Error: {error_msg}")

        media_id = publish_response.json().get('id')

        print(f"[instagram] âœ… SUCCESS! Video published to Instagram!")
