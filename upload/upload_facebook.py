...
def upload_to_facebook(video_path, description, title="Algorithm Visualization", thumb_path=None):
    ...
    if not access_token:
        error_msg = "âŒ FACEBOOK_ACCESS_TOKEN not set in environment variables"
        print(f"[facebook] {error_msg}")
        raise ValueError(error_msg)

    if not page_id:
        error_msg = "âŒ FACEBOOK_PAGE_ID not set in environment variables"
        print(f"[facebook] {error_msg}")
        raise ValueError(error_msg)
    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[facebook] âŒ UPLOAD FAILED!")
        ...
        print(f"[facebook] âŒ UNEXPECTED ERROR!")
...
def upload_to_facebook_story(video_path):
    ...
    """
    print("\n" + "=" * 60)
    print("ðŸ“˜ FACEBOOK STORY UPLOAD STARTING")
    print("=" * 60)
    ...
    try:
        ...
        if res_init.status_code != 200:
             print(f"[facebook] âŒ Init Error: {res_init.text}")
        ...
        if res_finish.status_code == 200 or res_finish.json().get('success'):
            print(f"[facebook] âœ… SUCCESS! Story uploaded!")
            print(f"[facebook] Video ID: {video_id}")
            print("=" * 60)
            return {'id': video_id, 'platform': 'facebook_story', 'status': 'success'}
        else:
            print(f"[facebook] âŒ Finish Phase Error: {res_finish.text}")
            raise Exception(f"Finish Phase Failed: {res_finish.text}")

    except Exception as e:
        print(f"[facebook] âŒ ERROR: {e}")
