...
def upload_to_threads(video_path, text):
    ...
    print("\n" + "=" * 60)
    print("ðŸ§µ THREADS UPLOAD STARTING")
    print("=" * 60)
    ...
    if not access_token:
        error_msg = "âŒ THREADS_ACCESS_TOKEN not set"
        print(f"[threads] {error_msg}")
        raise ValueError(error_msg)

    if not user_id:
        error_msg = "âŒ THREADS_USER_ID not set"
        print(f"[threads] {error_msg}")
        raise ValueError(error_msg)

    print(f"[threads] âœ… Credentials loaded")

    # Check video file
    video_path_obj = Path(video_path)
    if not video_path_obj.exists():
        error_msg = f"âŒ Video file not found: {video_path}"
        print(f"[threads] {error_msg}")
        raise FileNotFoundError(error_msg)

    file_size_mb = video_path_obj.stat().st_size / (1024 * 1024)
    print(f"[threads] âœ… Video file found: {video_path}")
    ...
    try:
        ...
        print(f"[threads] ðŸ“¤ Step 1: Uploading to temporary hosting...")
        ...
        try:
            ...
            if temp_response.status_code == 200:
                ...
                if temp_url:
                    ...
                    print(f"[threads] âœ… Uploaded to tmpfiles.org: {video_url}")
        except Exception as e:
             print(f"[threads] âš ï¸ tmpfiles.org exception: {e}")

        if not video_url:
             raise Exception("All hosting attempts failed (tmpfiles.org)")

        print(f"[threads] âœ… Temporary URL ready: {video_url}")

        # Step 2: Create Threads container with video URL
        print(f"[threads] ðŸ“¦ Step 2: Creating Threads container...")
        ...
        for api_version in api_versions:
            ...
            if container_response.status_code == 200:
                ...
                if container_id:
                    print(f"[threads] âœ… Container created with API {api_version}: {container_id}")
                    break
            else:
                ...
                print(f"[threads] âŒ API {api_version} failed:")
        ...
        if not container_id:
            ...
            print(f"[threads] âŒ {error_msg}")
            raise Exception(error_msg)

        # Step 3: Wait for processing
        print(f"[threads] â³ Step 3: Waiting for video processing...")
        max_wait = 120
        waited = 0
        ...
        while waited < max_wait:
            ...
            if status == 'FINISHED':
                print(f"[threads] âœ… Video processing complete!")
                break
            elif status == 'ERROR':
        ...
        # Step 4: Publish
        print(f"[threads] ðŸ“¤ Step 4: Publishing to Threads...")
        ...
        if publish_response.status_code != 200:
            ...
            error_msg = error_data.get('error', {}).get('message', 'Unknown error')
            print(f"[threads] âŒ Publish failed: {error_msg}")
            raise Exception(f"Threads Publish Error: {error_msg}")

        thread_id = publish_response.json().get('id')

        print(f"[threads] âœ… SUCCESS! Video published to Threads!")
