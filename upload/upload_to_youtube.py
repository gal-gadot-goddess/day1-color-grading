...
def get_authenticated_service():
    ...
    try:
        ...
        if "invalid_grant" in str(e).lower():
            print("\nâŒ [youtube] AUTH ERROR: Refresh token has EXPIRED or been REVOKED.")
            print("ðŸ’¡ SOLUTION: You must generate a NEW refresh token.")
...
def main():
    ...
    if not video_file.exists():
        print("[youtube] âŒ No video found at output/final_video.mp4")
        return
    ...
    try:
        ...
        print(f"[youtube] âŒ Upload failed: {e}")
        raise
